        ...
        <article class="<?php echo $show_sidebar_single ? 'lg:w-2/3' : 'lg:w-full'; ?> overflow-hidden bg-white p-6 sm:p-8 rounded-xl shadow-sm border border-gray-100">
            ...
            if ( ! empty( $related_posts ) ) :
                ...
                <div class="mt-16 border-t border-gray-100 pt-10">
                    <h3 class="text-2xl font-bold text-gray-900 mb-6"><?php _e( 'ข่าวที่เกี่ยวข้อง', 'chrysoberyl' ); ?></h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <?php foreach ( $related_posts as $related_post ) : ?>
                            <a href="<?php echo esc_url( chrysoberyl_fix_url( get_permalink( $related_post->ID ) ) ); ?>" class="group">
                                <div class="overflow-hidden rounded-xl mb-3 h-48">
                                    <?php
                                    if ( has_post_thumbnail( $related_post->ID ) ) {
                                        echo get_the_post_thumbnail( $related_post->ID, 'chrysoberyl-card', array(
