    ...
    while ( have_posts() ) :
        ...
        <article id="post-<?php the_ID(); ?>" <?php post_class( 'bg-white rounded-xl shadow-sm overflow-hidden' ); ?>>
                ...
                if ( ! empty( $gallery_images ) ) :
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" id="gallery-grid">
                        <?php foreach ( $gallery_images as $image_id ) : ?>
                            <?php
                            $image = chrysoberyl_get_attachment_image_src( $image_id, 'large' );
                            $full_image = chrysoberyl_get_attachment_image_src( $image_id, 'full' );
                            if ( $image ) :
                                ?>
            ...
            if ( $tags ) :
                ?>
                <div class="mb-8">
                    <h3 class="text-lg font-semibold mb-3"><?php _e( 'Tags:', 'chrysoberyl' ); ?></h3>
            ...
            if ( $related_galleries ) :
                ?>
                <div class="mt-12">
                    <h2 class="text-2xl font-bold mb-6"><?php _e( 'Related Galleries', 'chrysoberyl' ); ?></h2>
