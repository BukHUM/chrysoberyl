    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20">
                ...
                <?php else : ?>
                    <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="flex-shrink-0 flex items-center gap-3 group" aria-label="<?php bloginfo( 'name' ); ?>">
                        ...
                        <div class="flex flex-col justify-center -space-y-1">
                            ...
                            <span class="text-[10px] font-bold text-accent uppercase tracking-[0.2em] leading-tight"><?php _e( 'Chrysoberyl - Theme', 'chrysoberyl' ); ?></span>
                        </div>
                    </a>
                <?php endif; ?>
            </div>
            ...
            <div class="hidden md:flex items-center space-x-8">
                ...
                ?>
                    <button type="button" 
                            class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-accent hover:text-white transition chrysoberyl-search-toggle"
                            aria-label="<?php _e( 'ค้นหาข่าว', 'chrysoberyl' ); ?>">
                        <i class="fas fa-search text-sm"></i>
                    </button>
                <?php else : ?>
                    <div class="relative chrysoberyl-search-container">
                        <input type="search" 
                               class="chrysoberyl-search-input w-48 px-4 py-2 pl-10 pr-10 rounded-full border border-gray-200 focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent text-sm"
                               placeholder="<?php echo esc_attr( $search_placeholder ); ?>"
                               autocomplete="off" />
                        <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
                        <div class="chrysoberyl-search-suggestions absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 hidden max-h-96 overflow-y-auto"></div>
                    </div>
                <?php
                    endif;
                else :
