...
function chrysoberyl_add_meta_boxes() {
    ...
    add_meta_box(
        ...
        __( 'Post Options', 'chrysoberyl' ),
        'chrysoberyl_post_options_callback',
        'post',
        'side',
        'default'
    );
    ...
    foreach ( $post_types as $post_type ) {
        add_meta_box(
            ...
            __( 'Additional Information', 'chrysoberyl' ),
            'chrysoberyl_post_additional_callback',
            $post_type,
            'normal',
            'default'
        );
    ...
    foreach ( $post_types as $post_type ) {
        add_meta_box(
            ...
            __( 'SEO Options', 'chrysoberyl' ),
            'chrysoberyl_seo_options_callback',
            $post_type,
            'normal',
            'low'
        );
    ...
    add_meta_box(
        ...
        __( 'Video Options', 'chrysoberyl' ),
        'chrysoberyl_video_options_callback',
        'video_news',
        'normal',
        'high'
    );
    ...
    add_meta_box(
        ...
        __( 'Gallery Options', 'chrysoberyl' ),
        'chrysoberyl_gallery_options_callback',
        'gallery',
        'normal',
        'high'
    );
    ...
    add_meta_box(
        ...
        __( 'Featured Story Options', 'chrysoberyl' ),
        'chrysoberyl_featured_options_callback',
        'featured_story',
        'side',
        'default'
    );
 ...
 * Meta box callback
 */
function chrysoberyl_post_options_callback( $post ) {
    wp_nonce_field( 'chrysoberyl_save_meta', 'chrysoberyl_meta_nonce' );
 ...
 * Save meta box data
 */
function chrysoberyl_save_meta_box( $post_id ) {
    if ( ! isset( $_POST['chrysoberyl_meta_nonce'] ) || ! wp_verify_nonce( $_POST['chrysoberyl_meta_nonce'], 'chrysoberyl_save_meta' ) ) {
        return;
    }
...
add_action( 'save_post', 'chrysoberyl_save_meta_box' );

/**
 * Video News Options Meta Box Callback
 */
function chrysoberyl_video_options_callback( $post ) {
    wp_nonce_field( 'chrysoberyl_save_meta', 'chrysoberyl_meta_nonce' );
    ...
    <table class="form-table">
        <tr>
            <th><label for="video_url"><?php _e( 'Video URL', 'chrysoberyl' ); ?></label></th>
            <td>
                ...
                <p class="description"><?php _e( 'Enter the video URL (YouTube, Vimeo, etc.)', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label for="video_duration"><?php _e( 'Duration', 'chrysoberyl' ); ?></label></th>
            <td>
                ...
                <p class="description"><?php _e( 'Enter video duration (e.g., 5:30)', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label for="video_embed_code"><?php _e( 'Embed Code', 'chrysoberyl' ); ?></label></th>
            <td>
                ...
                <p class="description"><?php _e( 'Alternative: Paste embed code directly', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
    </table>
    <?php
 ...
 */
function chrysoberyl_gallery_options_callback( $post ) {
    wp_nonce_field( 'chrysoberyl_save_meta', 'chrysoberyl_meta_nonce' );
    ...
    <div class="gallery-images-container">
        <p>
            <button type="button" class="button button-secondary" id="add-gallery-images">
                <?php _e( 'Add Images', 'chrysoberyl' ); ?>
            </button>
        </p>
        <ul id="gallery-images-list" class="gallery-images-list">
            <?php foreach ( $gallery_images as $image_id ) : ?>
                <?php $image = chrysoberyl_get_attachment_image_src( $image_id, 'thumbnail' ); ?>
                <?php if ( $image ) : ?>
                    <li class="gallery-image-item" data-image-id="<?php echo esc_attr( $image_id ); ?>">
                        <img src="<?php echo esc_url( $image[0] ); ?>" alt="">
                        <button type="button" class="button-link remove-image"><?php _e( 'Remove', 'chrysoberyl' ); ?></button>
    ...
    jQuery(document).ready(function($) {
        ...
        $('#add-gallery-images').on('click', function(e) {
            ...
            frame = wp.media({
                title: '<?php _e( 'Select Gallery Images', 'chrysoberyl' ); ?>',
                button: {
                    text: '<?php _e( 'Add to Gallery', 'chrysoberyl' ); ?>'
                },
                multiple: true
            });
 ...
 */
function chrysoberyl_featured_options_callback( $post ) {
    wp_nonce_field( 'chrysoberyl_save_meta', 'chrysoberyl_meta_nonce' );
    ...
    <p>
        <label>
            <?php _e( 'Priority:', 'chrysoberyl' ); ?><br>
        ...
        <span class="description"><?php _e( 'Higher number = higher priority (1-10)', 'chrysoberyl' ); ?></span>
    </p>
    <p>
        <label>
            <?php _e( 'Expiry Date:', 'chrysoberyl' ); ?><br>
            <input type="date" name="featured_expiry" value="<?php echo esc_attr( $featured_expiry ); ?>" style="width: 100%;">
        </label>
        <span class="description"><?php _e( 'When should this stop being featured?', 'chrysoberyl' ); ?></span>
    </p>
    <?php
}
 ...
 */
function chrysoberyl_post_additional_callback( $post ) {
    wp_nonce_field( 'chrysoberyl_save_meta', 'chrysoberyl_meta_nonce' );
    ...
    <table class="form-table">
        <tr>
            <th><label for="custom_excerpt"><?php _e( 'Custom Excerpt', 'chrysoberyl' ); ?></label></th>
            <td>
                <textarea id="custom_excerpt" name="custom_excerpt" rows="4" class="large-text"><?php echo esc_textarea( $custom_excerpt ); ?></textarea>
                <p class="description"><?php _e( 'Override default excerpt. Leave empty to use default.', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label for="author_name"><?php _e( 'Author Name', 'chrysoberyl' ); ?></label></th>
            <td>
                <input type="text" id="author_name" name="author_name" value="<?php echo esc_attr( $author_name ); ?>" class="regular-text">
                <p class="description"><?php _e( 'Override default author name', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label for="author_bio"><?php _e( 'Author Bio', 'chrysoberyl' ); ?></label></th>
            <td>
                <textarea id="author_bio" name="author_bio" rows="3" class="large-text"><?php echo esc_textarea( $author_bio ); ?></textarea>
                <p class="description"><?php _e( 'Short author biography', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label for="featured_image_alt"><?php _e( 'Featured Image Alt Text', 'chrysoberyl' ); ?></label></th>
            <td>
                ...
                <p class="description"><?php _e( 'Alt text for featured image (for accessibility)', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label for="social_sharing_image"><?php _e( 'Social Sharing Image', 'chrysoberyl' ); ?></label></th>
            <td>
                <div class="social-image-preview" style="margin-bottom: 10px;">
                    <?php if ( $social_sharing_image ) : ?>
                        <?php $image = chrysoberyl_get_attachment_image_src( $social_sharing_image, 'medium' ); ?>
                        <?php if ( $image ) : ?>
                ...
                <button type="button" class="button" id="select-social-image"><?php _e( 'Select Image', 'chrysoberyl' ); ?></button>
                <button type="button" class="button" id="remove-social-image" style="<?php echo $social_sharing_image ? '' : 'display:none;'; ?>"><?php _e( 'Remove', 'chrysoberyl' ); ?></button>
                <p class="description"><?php _e( 'Image used for social media sharing (Open Graph, Twitter Card)', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label><?php _e( 'Related Posts', 'chrysoberyl' ); ?></label></th>
            <td>
                <div class="related-posts-container">
                    <button type="button" class="button" id="add-related-post"><?php _e( 'Add Related Post', 'chrysoberyl' ); ?></button>
                        ...
                        <?php foreach ( $related_posts as $related_post_id ) : ?>
                            ...
                            if ( $related_post ) :
                                ...
                                <li class="related-post-item" data-post-id="<?php echo esc_attr( $related_post_id ); ?>">
                                    ...
                                    <button type="button" class="button-link remove-related-post"><?php _e( 'Remove', 'chrysoberyl' ); ?></button>
    ...
    jQuery(document).ready(function($) {
        ...
        $('#select-social-image').on('click', function(e) {
            ...
            socialFrame = wp.media({
                title: '<?php _e( 'Select Social Sharing Image', 'chrysoberyl' ); ?>',
                button: {
                    text: '<?php _e( 'Use this image', 'chrysoberyl' ); ?>'
                },
                multiple: false
            });
        ...
        $('#add-related-post').on('click', function(e) {
            ...
            relatedFrame = wp.media({
                title: '<?php _e( 'Select Related Posts', 'chrysoberyl' ); ?>',
                button: {
                    text: '<?php _e( 'Add Posts', 'chrysoberyl' ); ?>'
                },
                multiple: true,
                library: {
                    type: 'post'
 ...
 */
function chrysoberyl_seo_options_callback( $post ) {
    wp_nonce_field( 'chrysoberyl_save_meta', 'chrysoberyl_meta_nonce' );
    ...
    <table class="form-table">
        <tr>
            <th><label for="meta_title"><?php _e( 'Meta Title', 'chrysoberyl' ); ?></label></th>
            <td>
                <input type="text" id="meta_title" name="meta_title" value="<?php echo esc_attr( $meta_title ); ?>" class="large-text" maxlength="60">
                <p class="description"><?php _e( 'SEO title (recommended: 50-60 characters)', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label for="meta_description"><?php _e( 'Meta Description', 'chrysoberyl' ); ?></label></th>
            <td>
                ...
                <p class="description"><?php _e( 'SEO description (recommended: 150-160 characters)', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
        <tr>
            <th><label for="meta_keywords"><?php _e( 'Meta Keywords', 'chrysoberyl' ); ?></label></th>
            <td>
                ...
                <p class="description"><?php _e( 'Comma-separated keywords', 'chrysoberyl' ); ?></p>
            </td>
        </tr>
    </table>
    <?php
