...
?>
    ...
    <footer class="bg-gradient-to-b from-gray-50 to-white border-t-2 border-gray-200 mt-16 pt-6 pb-6 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <?php if ( get_option( 'chrysoberyl_footer_newsletter_enabled', '1' ) === '1' ) : ?>
                    ...
                    <div class="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="font-bold text-xl text-gray-900 mb-1">
                                <?php _e( 'ไม่พลาดทุกเทรนด์', 'chrysoberyl' ); ?>
                            </h3>
                            <p class="text-gray-600 text-sm mb-0 md:mb-0">
                                <?php _e( 'สมัครรับข่าวสารสรุปประจำวันส่งตรงถึงอีเมลของคุณ', 'chrysoberyl' ); ?>
                            </p>
                        </div>
                        <div class="w-full md:w-auto md:flex-shrink-0">
                            <form class="flex flex-col sm:flex-row gap-2" onsubmit="event.preventDefault(); handleNewsletterSubmit(event);" aria-label="<?php _e( 'Newsletter subscription', 'chrysoberyl' ); ?>">
                                <input type="email" 
                                ...
                                <button type="submit"
                                    ...
                                    <i class="fas fa-paper-plane mr-2"></i><?php _e( 'สมัครรับข่าวสาร', 'chrysoberyl' ); ?>
                                </button>
                            </form>
                        </div>
                    </div>
                    ...
                    if ( $col_type === 'menu' && $col_menu ) {
                        ...
                        if ( $col_show_heading === '1' ) {
                            if ( $col_heading_text !== '' ) {
                                ...
                                $heading_label = $menu_obj && isset( $menu_obj->name ) ? $menu_obj->name : __( 'เมนู', 'chrysoberyl' );
                            }
                            ?>
                        ...
                        if ( is_active_sidebar( $sidebar_id ) ) {
                            ...
                            if ( $col_show_heading === '1' ) {
                                if ( $col_heading_text !== '' ) {
                                    ...
                                    $default_labels = array(
                                        1 => '',
                                        2 => __( 'หมวดหมู่', 'chrysoberyl' ),
                                        3 => __( 'เกี่ยวกับเรา', 'chrysoberyl' ),
                                        4 => __( 'ติดตามเรา', 'chrysoberyl' ),
                                    );
                                    $heading_label = isset( $default_labels[ $col ] ) ? $default_labels[ $col ] : sprintf( __( 'Footer %d', 'chrysoberyl' ), $col );
                                }
                                if ( $heading_label !== '' ) {
                                    ?>
                                    <h4 class="font-bold text-gray-900 text-base md:mb-5 group-hover:text-accent transition-colors">
                            ...
                            if ( $col === 1 ) {
                                ...
                                <button class="flex justify-between items-center w-full py-3 md:py-0 text-left md:cursor-default group" onclick="toggleFooter(this)" aria-expanded="false" aria-controls="footer-categories">
                                    <h4 class="font-bold text-gray-900 text-base md:mb-5 group-hover:text-accent transition-colors"><i class="fas fa-folder-open text-accent mr-2"></i><?php _e( 'หมวดหมู่', 'chrysoberyl' ); ?></h4>
                                ...
                                <button class="flex justify-between items-center w-full py-3 md:py-0 text-left md:cursor-default group" onclick="toggleFooter(this)" aria-expanded="false" aria-controls="footer-about">
                                    <h4 class="font-bold text-gray-900 text-base md:mb-5 group-hover:text-accent transition-colors"><i class="fas fa-info-circle text-accent mr-2"></i><?php _e( 'เกี่ยวกับเรา', 'chrysoberyl' ); ?></h4>
                                    <i class="fas fa-chevron-down text-gray-400 text-sm md:hidden transition-transform duration-300 transform"></i>
                                </button>
                                <?php
                                if ( has_nav_menu( 'footer' ) ) {
                                    ...
                                    <ul id="footer-about" class="footer-links space-y-2.5 text-sm text-gray-600 hidden md:block" role="list">
                                        <li><a href="#" class="footer-link hover:text-accent transition-colors duration-200 inline-flex items-center gap-2"><i class="fas fa-chevron-right text-xs text-gray-400"></i><?php _e( 'ติดต่อโฆษณา', 'chrysoberyl' ); ?></a></li>
                                        <li><a href="#" class="footer-link hover:text-accent transition-colors duration-200 inline-flex items-center gap-2"><i class="fas fa-chevron-right text-xs text-gray-400"></i><?php _e( 'ร่วมงานกับเรา', 'chrysoberyl' ); ?></a></li>
                                        <li><a href="#" class="footer-link hover:text-accent transition-colors duration-200 inline-flex items-center gap-2"><i class="fas fa-chevron-right text-xs text-gray-400"></i><?php _e( 'นโยบายความเป็นส่วนตัว', 'chrysoberyl' ); ?></a></li>
                                        <li><a href="#" class="footer-link hover:text-accent transition-colors duration-200 inline-flex items-center gap-2"><i class="fas fa-chevron-right text-xs text-gray-400"></i><?php _e( 'เงื่อนไขการใช้งาน', 'chrysoberyl' ); ?></a></li>
                                    </ul>
                                    <?php
                                }
                            } else {
                                ...
                                <button class="flex justify-between items-center w-full py-3 md:py-0 text-left md:cursor-default group" onclick="toggleFooter(this)" aria-expanded="false" aria-controls="footer-social">
                                    <h4 class="font-bold text-gray-900 text-base md:mb-5 group-hover:text-accent transition-colors"><i class="fas fa-share-alt text-accent mr-2"></i><?php _e( 'ติดตามเรา', 'chrysoberyl' ); ?></h4>
                                    <i class="fas fa-chevron-down text-gray-400 text-sm md:hidden transition-transform duration-300 transform"></i>
                                </button>
                                <div id="footer-social" class="footer-links hidden md:block">
                                    <p class="text-gray-600 text-sm mb-4"><?php _e( 'ติดตามข่าวสารและอัปเดตล่าสุด', 'chrysoberyl' ); ?></p>
                                    <div class="flex flex-wrap gap-3">
                                        <a href="#" class="footer-social-icon w-12 h-12 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-white hover:from-blue-700 hover:to-blue-800 transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-110" aria-label="<?php _e( 'Facebook', 'chrysoberyl' ); ?>" target="_blank" rel="noopener noreferrer"><i class="fab fa-facebook-f text-base"></i></a>
                                        <a href="#" class="footer-social-icon w-12 h-12 rounded-full bg-gradient-to-br from-sky-500 to-sky-600 flex items-center justify-center text-white hover:from-sky-600 to-sky-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-110" aria-label="<?php _e( 'Twitter', 'chrysoberyl' ); ?>" target="_blank" rel="noopener noreferrer"><i class="fab fa-twitter text-base"></i></a>
                                        <a href="#" class="footer-social-icon w-12 h-12 rounded-full bg-gradient-to-br from-pink-600 to-pink-700 flex items-center justify-center text-white hover:from-pink-700 to-pink-800 transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-110" aria-label="<?php _e( 'Instagram', 'chrysoberyl' ); ?>" target="_blank" rel="noopener noreferrer"><i class="fab fa-instagram text-base"></i></a>
            ...
            <?php if ( get_option( 'chrysoberyl_footer_tags_enabled', '1' ) === '1' ) : ?>
                ...
                <div class="flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-gray-500">
                    <div class="text-center md:text-left">
                        ...
                        if ( $copyright_text !== '' ) {
                            ...
                            _e( 'All rights reserved.', 'chrysoberyl' );
                        }
                        ?>
                    </div>
                    <div class="flex items-center gap-6 text-xs">
                        ...
                        if ( has_nav_menu( 'footer_copyright' ) ) {
                            ...
                            ?>
                            <a href="#" class="hover:text-accent transition-colors"><?php _e( 'Privacy Policy', 'chrysoberyl' ); ?></a>
                            <span class="text-gray-300">|</span>
                            <a href="#" class="hover:text-accent transition-colors"><?php _e( 'Terms of Service', 'chrysoberyl' ); ?></a>
                            <span class="text-gray-300">|</span>
                            <a href="#" class="hover:text-accent transition-colors"><?php _e( 'Contact', 'chrysoberyl' ); ?></a>
                            <?php
                        }
                        ?>
                    </div>
    ...
    <button id="back-to-top" 
            ...
            aria-label="<?php _e( 'กลับขึ้นด้านบน', 'chrysoberyl' ); ?>"
