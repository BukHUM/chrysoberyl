...
<main id="main-content" class="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
    ...
    <?php $show_sidebar_single = ( get_option( 'chrysoberyl_sidebar_single_page_enabled', '1' ) === '1' ); ?>
            ...
            while ( have_posts() ) :
                    ...
                    <div class="flex items-center gap-2 mb-4">
                        <span class="text-gray-500 text-sm">
                            ...
                            if ( get_the_modified_date() !== get_the_date() ) {
                                printf( __( 'อัปเดตเมื่อ %s', 'chrysoberyl' ), esc_html( $updated ) );
                            } else {
                                printf( __( 'เผยแพร่เมื่อ %s', 'chrysoberyl' ), esc_html( get_the_date() ) );
                            }
                            ?>
                        </span>
                    </div>
                ...
                <?php if ( has_post_thumbnail() ) : ?>
                    <figure class="mb-8 rounded-xl overflow-hidden shadow-lg">
                        <?php the_post_thumbnail( 'chrysoberyl-hero', array( 'class' => 'w-full h-auto object-cover' ) ); ?>
                ...
                <?php
                if ( get_option( 'chrysoberyl_social_show_on_page', '1' ) === '1' ) :
                    $display_positions_bottom = get_option( 'chrysoberyl_social_display_positions', array( 'single_bottom' ) );
